...
const ReelInsightsScreen = () => {
  ...
  const [editFollowerPct, setEditFollowerPct] = useState(
    ins?.followerViewsPct && ins.followerViewsPct > 0 ? ins.followerViewsPct : 36
  );
  const [editGenderMale, setEditGenderMale] = useState(
    ins?.genderMale && ins.genderMale > 0 ? ins.genderMale : 56
  );
    ...
    <div id="insights-main" className="pb-20 min-h-screen bg-background">
      ...
      <div className="flex border-b border-border">
        {(["Overview", "Engagement", "Audience"] as const).map((tab) => (
          ...
          >
            {tab}
            {activeTab === tab && <div className="absolute bottom-0 left-1/2 -translate-x-1/2 h-[2px] w-[97px] bg-foreground" />}
          </button>
        ))}
      </div>
          ...
          <div className="px-4 py-5">
            ...
            <div className="grid grid-cols-2 gap-2">
              {[
                ...
                <div 
                  key={item.label}
                  className={cn("bg-[#EEF0F2] dark:bg-secondary rounded-xl p-4", isEditMode && "cursor-pointer active:opacity-60")}
          ...
          <div className="px-4 pt-4 pb-2">
            ...
            <div className="space-y-1">
              {[
                ...
                >
                  <div className="flex items-center gap-3">
                    <div className="w-9 h-9 rounded-full bg-[#EEF0F2] dark:bg-secondary flex items-center justify-center">
