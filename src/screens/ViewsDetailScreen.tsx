        ...
        <div className="py-7">
          ...
          <div className="flex gap-4 overflow-x-auto hide-scrollbar px-4 pb-4">
            ...
            <div className="flex-shrink-0 w-[280px] bg-card rounded-[16px] border border-border p-4">
                <h4 className="text-[16px] font-bold text-foreground mb-5">Top towns/cities</h4>
                <div className="space-y-5">
                    {data.cities.map((city, i) => (
                        <div key={city.name}>
                            <p className="text-[14px] font-semibold text-foreground mb-1.5">{city.name}</p>
                            <div className="flex items-center gap-3">
                                <div className="flex-1 h-[10px] bg-secondary rounded-full overflow-hidden">
                                    <div className="h-full bg-[#D32FE0] rounded-full" style={{ width: `${Math.max(city.pct * 3, 4)}%` }} />
                                </div>
                                <span className="text-[13px] font-semibold text-foreground w-10 text-right">{city.pct}%</span>
                            </div>
                            {isEditing && (
                                <div className="mt-2 grid grid-cols-2 gap-2">
                                    <input className="bg-secondary rounded px-2 py-1 text-[11px] font-bold outline-none text-foreground" value={city.name} onChange={e => {
                                        const n = [...data.cities]; n[i].name = e.target.value; updateField('cities', n);
                                    }} />
                                    <input className="bg-secondary rounded px-2 py-1 text-[11px] font-bold outline-none text-foreground" type="number" value={city.pct} onChange={e => {
            ...
            {/* Countries */ }
            <div className="flex-shrink-0 w-[280px] bg-card rounded-[16px] border border-border p-4">
                <h4 className="text-[16px] font-bold text-foreground mb-5">Top countries</h4>
                <div className="space-y-5">
                    {data.countries.map((country, i) => (
                        <div key={country.name}>
                            <p className="text-[14px] font-semibold text-foreground mb-1.5">{country.name}</p>
                            <div className="flex items-center gap-3">
                                <div className="flex-1 h-[10px] bg-secondary rounded-full overflow-hidden">
                                    <div className="h-full bg-[#D32FE0] rounded-full" style={{ width: `${Math.max(country.pct, 4)}%` }} />
                                </div>
                                <span className="text-[13px] font-semibold text-foreground w-10 text-right">{country.pct}%</span>
                            </div>
                            {isEditing && (
                                <div className="mt-2 grid grid-cols-2 gap-2">
                                    <input className="bg-secondary rounded px-2 py-1 text-[11px] font-bold outline-none text-foreground" value={country.name} onChange={e => {
                                        const n = [...data.countries]; n[i].name = e.target.value; updateField('countries', n);
                                    }} />
                                    <input className="bg-secondary rounded px-2 py-1 text-[11px] font-bold outline-none text-foreground" type="number" value={country.pct} onChange={e => {
            ...
            {/* Age Ranges */}
            <div className="flex-shrink-0 w-[280px] bg-card rounded-[16px] border border-border p-4">
                <h4 className="text-[16px] font-bold text-foreground mb-5">Top age ranges</h4>
                <div className="space-y-5">
                    {data.ageRanges.map((range, i) => (
                        <div key={range.range}>
                            <p className="text-[14px] font-semibold text-foreground mb-1.5">{range.range}</p>
                            <div className="flex items-center gap-3">
                                <div className="flex-1 h-[10px] bg-secondary rounded-full overflow-hidden">
                                    <div className="h-full bg-[#D32FE0] rounded-full" style={{ width: `${range.pct}%` }} />
                                </div>
                                <span className="text-[13px] font-semibold text-foreground w-10 text-right">{range.pct}%</span>
                            </div>
                            {isEditing && (
                                <div className="mt-2 grid grid-cols-2 gap-2">
                                    <input className="bg-secondary rounded px-2 py-1 text-[11px] font-bold outline-none text-foreground" value={range.range} onChange={e => {
                                        const n = [...data.ageRanges]; n[i].range = e.target.value; updateField('ageRanges', n);
                                    }} />
                                    <input className="bg-secondary rounded px-2 py-1 text-[11px] font-bold outline-none text-foreground" type="number" value={range.pct} onChange={e => {
